...
@app_base.get("/fake_products/{count}")
async def create_note(count: int):
    for i in range(count):
...
@app_base.get("/fake_orders/{count}")
async def create_note(count: int):
    for i in range(count):
...
@app_base.get("/users/", response_model=List[models.UserRead])
async def read_users():
    query = db.users.select()
    return await db.database.fetch_all(query)


@app_base.get("/products/", response_model=List[models.ProductRead])
async def read_products():
    query = db.products.select()
    return await db.database.fetch_all(query)


@app_base.get("/orders/", response_model=List[models.OrderRead])
...
@app_base.get("/users/{user_id}", response_model=models.UserRead)
...
@app_base.get("/products/{product_id}", response_model=models.ProductRead)
...
@app_base.get("/orders/{order_id}", response_model=models.OrderRead)
...
@app_base.put("/users/{user_id}", response_model=models.UserRead)
...
@app_base.put("/products/{product_id}", response_model=models.ProductRead)
...
@app_base.put("/orders/{order_id}", response_model=models.OrderRead)
...
@app_base.delete("/products/{product_id}")
...
@app_base.delete("/orders/{order_id}")
